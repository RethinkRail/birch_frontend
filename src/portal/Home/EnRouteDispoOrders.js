...
const Home = () => {
    ...
    const loadMoreWorkOrders100And125 = async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_BIRCH_API_URL}get_active_workorder_100_125`);
    ...
    const loadMoreDispoWorkOrders= async () => {
        try {
            const response = await axios.get(`${process.env.REACT_APP_BIRCH_API_URL}get_active_workorder_in_dispo`);
            const allWorkOrders = response.data.active_workorder;
            setWorkOrders(prevWorkOrders => [...prevWorkOrders, ...allWorkOrders]);
        } catch (error) {
            console.error("Error loading work orders:", error);
        }
    };
...
//                console.log(response.data)
    ...
    //     getAllStatusCode()
    //         .then(() => loadMoreWorkOrders100And125())
            ...
            <div className="font-inter">
                {workOrders.length > 0 ? (
                    <WorkOrderDataTable
                        ...
                        title={"Enroute And Dispo Orders"}
                    />
                ) : null}
            </div>
