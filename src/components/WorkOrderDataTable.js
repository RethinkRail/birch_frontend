...
const WorkOrderDataTable =({workOrders,statusCode,updateWorkUpdates,updateMaterialETA,updatePOD,updateMarkAsFinalized,updateMarkAsShipped}) =>{
    ...
    const workOrdersTableColumn = [
        ...
        },
        {
            ...
            className: "pl-5",
            cell: (row) => (
                <select className={`w-32 ${row.index % 2 === 0 ? '' : 'bg-[#F7F9FF]'}`}>
                    {statusCode.map((sc) => (
        ...
        },
        {
            ...
            cell: (row) => (
                ...
                }} className="cursor-pointer w-[198px] max-w-[198px] overflow-hidden whitespace-pre-line  text-ellipsis">{row.comment[0].comment}</span>
            ),
            sortable: true
        },
        {
        ...
        },
        {
            ...
            cell:(row)=>(
                <input
                    type="checkbox"
                    // onChange={() => handleOrderCheckboxChange(index, "FINALIZED")}
                    checked={row.shipped !== "1900-01-01T00:00:00.000Z"}
                    className=" checkbox checkbox-primary mt-[6px]  w-[20px] h-[20px]" />
            )
        },
        {
            name: "ACTION",
    ...
    const myStyles =
        {
            headRow:{
                style: {"backgroundColor":"#DCE5FF","font-size":"11px","padding-right":"1px"},
            },
            cells:{
