...
// Function to sort nested arrays by "name"
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (!editRowData) {
                ...
                if(selectedTable==='jobcode' || selectedTable==='conditioncode' || selectedTable==='locationcode'){
    ...
    return (
        ...
        >
            <div className='bg-white rounded-md shadow-md grid grid-cols-2 gap-1.5 p-3'
                 onClick={(e) => e.stopPropagation()}>
                {relatedData && fields.filter(field => field.Field.toLowerCase() !== "id").map((field, index) => (
                    <div key={`field--edit--${index}`}
                         className={`${field.Field.toLowerCase() === "id" || (editRowData && field.Field.toLowerCase() === "updated_by") || (field.Field.toLowerCase() === "last_update") ? "hidden" : ""} col-span-1 flex justify-center gap-1 flex-col`}>
                        <label htmlFor={field.Field.toLowerCase()}
                               className='capitalize text-[12px]'>{field.Field.split("_").join(" ").toUpperCase()}:</label>
                        {tableSchema.find(schema => schema.name === field.Field && schema.isForeignKey) ? (
                            ...
                            >
                                ...
                                {relatedData[field.Field] && relatedData[field.Field].map(option => {
                                    ...
                                    return (
                                        <option key={option[foreignKeyField]}
                                                value={option[foreignKeyField]}>{foreignKeyField === "id" ? option?.name : option[foreignKeyField]}</option>
                                    );
                                })}
                            </select>
                        ) : (
